...
import { FETCH_LOCATION, FETCH_LOCATION_FUTURE, CLICK_LOCATION } from './constants';
...
function* fetchAsync(props) {
  try {
    ...
    yield put(
      response.data.error ? onRequestError(response.data.error) : onRequestSuccess(response),
    );
  } catch (error) {
...
function* fetchAsyncFuture(props) {
  try {
    ...
    const response = yield call(() =>
      axios.get(
        `http://api.weatherstack.com/current?access_key=e65fcbdb6b7edea6d370e4fd261bf357&query=${props.search}&forecast_days=1&hourly=1`,
      ),
    );
    yield put(
      response.data.error ? onRequestError(response.data.error) : onRequestSuccessHistory(response),
    );
  } catch (error) {
    console.log(error.message);
  }
